...
function renderCards(cards) {
  ...
  if (cards.length === 0) {
    ...
    noResults.textContent = "Извините, товары по вашему запросу не найдены";
  ...
  cards.forEach((element) => {
    ...
    button.className = "container-for-button-for-last3cardmets";
    card.appendChild(button);
    const text_button = document.createElement("p");
    text_button.className = "itame-for-button-for-last3cardmets";
    button.appendChild(text_button);
    text_button.textContent = "Add to card";
...
function sortCategory(category) {
  ...
  for (let i = 0; i < cards.length; i++) {
    if (cards[i].category == category) {
      category_cards.push(cards[i]);
    }
  }
  renderCards(category_cards);
}
...
section.appendChild(container_buttons);
tasks.forEach((task) => {
  const btn = document.createElement("button");
  btn.textContent = task.name;
  btn.className = "my-custom-button";
  btn.addEventListener("click", task.action);
  container_buttons.appendChild(btn);
});
